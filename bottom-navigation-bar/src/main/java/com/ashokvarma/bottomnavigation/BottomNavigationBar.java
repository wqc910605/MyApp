...
public class BottomNavigationBar extends FrameLayout {
    ...
    public static final int MODE_STILL = 5;//静止不动, 没有动画
    public static final int MODE_STILL_NO_TITLE = 6;//静止不动, 没有动画, 没有标题


    @IntDef({MODE_STILL, MODE_STILL_NO_TITLE, MODE_DEFAULT, MODE_FIXED, MODE_SHIFTING, MODE_FIXED_NO_TITLE, MODE_SHIFTING_NO_TITLE})
    ...
    public static final int BACKGROUND_STYLE_STILL = 3;//固定模式

    @IntDef({BACKGROUND_STYLE_STILL, BACKGROUND_STYLE_DEFAULT, BACKGROUND_STYLE_STATIC, BACKGROUND_STYLE_RIPPLE})
    @Retention(RetentionPolicy.SOURCE)
    ...
    private void parseAttrs(Context context, AttributeSet attrs) {
        if (attrs != null) {
            ...
            switch (typedArray.getInt(R.styleable.BottomNavigationBar_bnbBackgroundStyle, BACKGROUND_STYLE_DEFAULT)) {
                case BACKGROUND_STYLE_STILL://静止不动的模式
                    mBackgroundStyle = BACKGROUND_STYLE_STILL;
                    break;
...
//    }
    ...
    public void initialise() {
        ...
        if (!mBottomNavigationItems.isEmpty()) {
            ...
            if (mMode == MODE_STILL && mBackgroundStyle == BACKGROUND_STYLE_STILL) {
                mBackgroundOverlay.setVisibility(View.GONE);
                mContainer.setBackgroundColor(mBackgroundColor);
            }

            int screenWidth = Utils.getScreenWidth(getContext());
            if (mMode == MODE_STILL || mMode == MODE_STILL_NO_TITLE) {
                int[] widths = BottomNavigationHelper.getMeasurementsForStillMode(getContext(), screenWidth, mBottomNavigationItems.size(), mScrollable);
                int itemWidth = widths[0];

                for (BottomNavigationItem currentItem : mBottomNavigationItems) {
                    StillBottomNavigationTab bottomNavigationTab = new StillBottomNavigationTab(getContext());
                    setUpTab(mMode == MODE_STILL_NO_TITLE, bottomNavigationTab, currentItem, itemWidth, itemWidth);
                }
            }
    ...
    private void setUpTab(boolean isNoTitleMode, BottomNavigationTab bottomNavigationTab, BottomNavigationItem currentItem, int itemWidth, int itemActiveWidth) {
        ...
        bottomNavigationTab.initialise(mBackgroundStyle == BACKGROUND_STYLE_STATIC || mBackgroundStyle == BACKGROUND_STYLE_STILL);
    ...
    private void selectTabInternal(int newPosition, boolean firstTab, boolean callListener, boolean forcedSelection) {
        ...
        if (mSelectedPosition != newPosition) {
            if (mBackgroundStyle == BACKGROUND_STYLE_STILL) {
